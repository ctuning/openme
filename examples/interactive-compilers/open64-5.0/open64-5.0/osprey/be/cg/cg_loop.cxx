...
extern void *Record_And_Del_Loop_Region(LOOP_DESCR *loop, void *tmp);
  ...
  case SINGLE_BB_DOLOOP_SWP:
	...
	if (trace_loop_opt) 
      ...
      //FGG
      alc_unroll.factor=cg_loop.Unroll_factor();
      openme_callback("ALC_TRANSFORM_UNROLL", &alc_unroll);

      if (alc_unroll.factor > 1) {
        Unroll_Do_Loop(cg_loop, alc_unroll.factor);
	cg_loop.Recompute_Liveness();
      }
  ...
  case SINGLE_BB_WHILELOOP_UNROLL:
	...
	CG_LOOP_Trace_Loop(loop, "*** Before SINGLE_BB_WHILELOOP_UNROLL ***");
      ...
      Unroll_Dowhile_Loop(loop, alc_unroll.factor);
