...
unroll_and_peel_loops (int flags)
{
  ...
  alc_unroll.func_name="";
  if(current_function_decl != NULL)
  {
     strcpy(func_name, lang_hooks.decl_printable_name(current_function_decl, 2));
     alc_unroll.func_name=func_name;
  }
  ...
  FOR_EACH_LOOP (li, loop, LI_FROM_INNERMOST)
    {
      // FGG
      printf("FGG0=%s, %u\n\n", alc_unroll.func_name, loop->num);
      sprintf(loop_num, "%u", loop->num);
      alc_unroll.loop_name=loop_num;
      openme_callback("ALC_TRANSFORM_UNROLL_INIT", &alc_unroll);

      alc_obj.json=alc_unroll.json;

      sprintf(alc_buf, "%u", PARAM_VALUE (PARAM_MAX_UNROLLED_INSNS));
      alc_obj.name="PARAM_MAX_UNROLLED_INSNS";
      alc_obj.param=alc_buf;
      openme_callback("ALC_TRANSFORM_UNROLL_FEATURES", &alc_obj);

      sprintf(alc_buf, "%u", loop->ninsns);
      alc_obj.name="ninsns";
      alc_obj.param=alc_buf;
      openme_callback("ALC_TRANSFORM_UNROLL_FEATURES", &alc_obj);

      sprintf(alc_buf, "%u", PARAM_VALUE (PARAM_MAX_AVERAGE_UNROLLED_INSNS));
      alc_obj.name="PARAM_MAX_AVERAGE_UNROLLED_INSNS";
      alc_obj.param=alc_buf;
      openme_callback("ALC_TRANSFORM_UNROLL_FEATURES", &alc_obj);

      sprintf(alc_buf, "%u", loop->av_ninsns);
      alc_obj.name="av_ninsns";
      alc_obj.param=alc_buf;
      openme_callback("ALC_TRANSFORM_UNROLL_FEATURES", &alc_obj);

      sprintf(alc_buf, "%u", PARAM_VALUE (PARAM_MAX_UNROLL_TIMES));
      alc_obj.name="PARAM_MAX_UNROLL_TIMES";
      alc_obj.param=alc_buf;
      openme_callback("ALC_TRANSFORM_UNROLL_FEATURES", &alc_obj);
	...
	case LPT_UNROLL_CONSTANT:
	  // FGG
          printf("FGG0a\n");
          alc_unroll.factor=loop->lpt_decision.times;
          openme_callback("ALC_TRANSFORM_UNROLL", &alc_unroll);
          loop->lpt_decision.times=alc_unroll.factor;
